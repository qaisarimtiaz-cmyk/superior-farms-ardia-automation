...
export async function run(browser: Browser, testInfo: any = null) {
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════════
    console.log('Step 23: Opening and logging into Ardia...');
...
async function openViaSearch(page: Page, term: string, optionName: RegExp, gridTimeout: number): Promise<void> {
  ...
  // The grid's visibility below is the real "page is ready" signal.
  await page.waitForLoadState('domcontentloaded', { timeout: t.navigation }).catch(() => {});

  // D365 has two grid types and this helper opens BOTH kinds of pages:
  //   - React grids (On-hand, Cold scale): .reactGrid, [id*="MainGrid"]
  //   - Standard form grids (Report as finished staging / F3ProdAppRAFProcessForm):
  //     rendered as plain rows, NO .reactGrid / [id*="MainGrid"] element.
  // The old code only waited for react-grid selectors, so on a standard form
  // grid it waited the FULL gridTimeout (~2 min) and appeared "stuck", then
  // threw. We try react-grid selectors briefly, then fall back to the
  // standard-form row/section selectors, then to networkidle as a last resort.
...
async function filterStagingByProduction(page: Page, batchOrderId: string): Promise<void> {
  ...
  // Don't wait for 'networkidle' on the live staging grid (it rarely settles) —
  // a short settle is enough; the row/IsSync checks have their own waits.
